    ...
    <section className="mx-auto max-w-5xl rounded-3xl border border-white/10 bg-white/[0.06] p-6 shadow-2xl shadow-black/30 sm:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          {episodeTitle ? (
            <p className="mt-3 text-lg text-zinc-300">{episodeTitle}</p>
          ) : null}
        </div>
        <span className="rounded-full border border-white/15 bg-white/10 px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] text-zinc-100">
          {status}
        </span>
      </div>

      <div className="mt-6 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-center">
        <p className="break-all rounded-2xl bg-black/30 p-4 font-mono text-sm text-zinc-200">
          {runId}
        </p>
      ...
      {steps.length ? (
        ...
        <p className="mt-8 rounded-2xl bg-white/[0.05] p-5 text-zinc-300">
